    ...
    levels:{
      easy:{
        ...
        questions:[
          ...
          {q:"מה הוא ReplicaSet?",options:["גיבוי לPods","חיבור Pods לService","קונטרולר שמבטיח מספר Pods מסוים רץ בכל עת","בסיס נתונים לstate"],answer:2,explanation:"ReplicaSet מבטיח שמספר ספציפי של Pods זהים רץ תמיד. Deployment מנהל ReplicaSets ומוסיף יכולות rolling updates ו-rollback."},
          ...
          {q:"מה ההבדל בין Label ל-Annotation?",options:["Label מוצפן, Annotation לא","Annotation ניתן לשימוש ב-selector בלבד","Label לא ניתן לעריכה","Label לתיוג ובחירה (selector), Annotation למטא-דטה שלא משמש לselector"],answer:3,explanation:"Labels הם key-value לתיוג וסינון ב-selectors. Annotations שומרים metadata נוסף (URLs, owners) – לא ניתן להשתמש בהם ב-selectors."},
          {q:"מה ההבדל בין kubectl apply ל-kubectl create?",options:["אין הבדל","apply מעדכן resource קיים; create נכשל אם resource קיים","create מהיר יותר","apply מחייב גרסה ספציפית"],answer:1,explanation:"kubectl create נכשל אם resource קיים. kubectl apply יוצר אם לא קיים, או מעדכן אם קיים (idempotent). מומלץ apply ב-CI/CD."},
          {q:"מה imagePullPolicy: Always גורם ל-Kubernetes לעשות?",options:["משתמש תמיד בimage מה-cache המקומי","לא מוריד image אם קיים","מוריד image מה-registry בכל פעם שPod מתזמן","מוריד רק אם ה-tag השתנה"],answer:2,explanation:"imagePullPolicy: Always מאלץ את kubelet למשוך image מה-registry בכל הפעלה. IfNotPresent (ברירת מחדל) מוריד רק אם לא קיים מקומית."},
          {q:"מה שלבי ה-Pod האפשריים ב-Kubernetes?",options:["Pending, Running, Succeeded, Failed, Unknown","Starting, Active, Done, Error","Init, Running, Stopped, Crashed","Created, Running, Terminated"],answer:0,explanation:"חמשת שלבי Pod: Pending (מתזמן/מוריד image), Running (קונטיינר רץ), Succeeded (כולם יצאו 0), Failed (אחד נכשל), Unknown (Node לא מדווח)."},
          {q:"מה terminationGracePeriodSeconds מגדיר?",options:["זמן בין log entries","זמן מקסימלי ללiveness probe","timeout לreadiness probe","זמן שK8s ממתין אחרי SIGTERM לפני SIGKILL"],answer:3,explanation:"כשPod נמחק, K8s שולח SIGTERM ומחכה terminationGracePeriodSeconds (ברירת מחדל: 30s) לפני SIGKILL. מאפשר סגירת connections graceful."},
          {q:"מה מאפיין Pod עם מספר קונטיינרים?",options:["כל קונטיינר מקבל IP נפרד","הקונטיינרים חולקים IP, network namespace, ו-volumes","לא נתמך בproduction","כל קונטיינר ב-Namespace נפרד"],answer:1,explanation:"קונטיינרים באותו Pod חולקים IP, network namespace, ו-volumes. הם תקשרים ישירות דרך localhost. דפוס נפוץ: main app + sidecar."},
          {q:"לאיזה מטרה משמש kubectl get pod <name> -o yaml?",options:["מוחק את ה-Pod","מראה רק שגיאות","מציג את המפרט המלא כ-YAML לdebug וייצוא","מפעיל Pod מחדש"],answer:2,explanation:"-o yaml מציג את ה-Pod spec המלא כולל status, הגדרות, ו-metadata. שימושי לחקירת הגדרות, ייצוא, ויצירת משאב דומה."},
          {q:"מה ההבדל בין קונטיינר ל-Pod ב-Kubernetes?",options:["Pod הוא wrapper שמכיל קונטיינר אחד+ עם רשת, storage, ומחזור חיים משותף","Pod הוא קונטיינר קטן יותר","Pod לproduction, קונטיינר לdev","Pod הוא image, קונטיינר הוא instance"],answer:0,explanation:"קונטיינר הוא יחידת ריצה מבודדת. Pod הוא abstraction של K8s שמנהל קונטיינר אחד+ עם IP משותף, storage, ומחזור חיים."},
        ],
        questionsEn:[
          ...
          {q:"What is a ReplicaSet in Kubernetes?",options:["A Pod backup mechanism","A connector between Pods and Services","A controller ensuring a specified number of identical Pods always run","A state database for Pods"],answer:2,explanation:"A ReplicaSet ensures a specific number of Pod replicas run at all times. A Deployment manages ReplicaSets and adds rolling update and rollback capabilities."},
          {q:"What is the purpose of a Namespace in Kubernetes?",options:["Logical isolation of resources for environments, teams, and projects","Storing logs","A type of Service","Storing container images"],answer:0,explanation:"Namespaces provide logical separation. Resources in different Namespaces are isolated. Commonly used for: dev/staging/prod, different teams, or multi-tenant clusters."},
          {q:"What is the difference between a Label and an Annotation?",options:["Labels are encrypted, Annotations are not","Only Annotations can be used in selectors","They are identical","Labels are for tagging and selection via selectors; Annotations store metadata not used in selectors"],answer:3,explanation:"Labels are key-value pairs for tagging and filtering via selectors (Services, Deployments). Annotations store extra metadata (URLs, owners, build IDs) — not usable in selectors."},
          {q:"What is the difference between kubectl apply and kubectl create?",options:["No difference","apply updates an existing resource; create fails if the resource already exists","create is faster","apply requires a specific version"],answer:1,explanation:"kubectl create fails if the resource already exists. kubectl apply creates or updates the resource (idempotent). Apply is the recommended approach in CI/CD pipelines."},
          {q:"What does imagePullPolicy: Always cause Kubernetes to do?",options:["Always use the locally cached image","Skip pulling if image exists locally","Pull the image from the registry every time a Pod is scheduled","Pull only if the tag changed"],answer:2,explanation:"imagePullPolicy: Always forces kubelet to pull the image from the registry on every container start. IfNotPresent (default) only pulls if the image isn't cached locally."},
          {q:"What are the possible Pod phases in Kubernetes?",options:["Pending, Running, Succeeded, Failed, Unknown","Starting, Active, Done, Error","Init, Running, Stopped, Crashed","Created, Running, Terminated"],answer:0,explanation:"The five Pod phases: Pending (scheduling/pulling), Running (container active), Succeeded (all exited 0), Failed (at least one non-zero exit), Unknown (node not reporting)."},
          {q:"What does terminationGracePeriodSeconds define?",options:["Time between log entries","Maximum time for a liveness probe","Timeout for a readiness probe","Time Kubernetes waits after SIGTERM before sending SIGKILL"],answer:3,explanation:"When a Pod is deleted, K8s sends SIGTERM and waits terminationGracePeriodSeconds (default 30s) before SIGKILL, allowing the app to close connections gracefully."},
          {q:"What characterises a multi-container Pod?",options:["Each container gets a separate IP","Containers share IP, network namespace, and volumes","Not supported in production","Each container runs in a separate Namespace"],answer:1,explanation:"Containers in the same Pod share: IP address, network namespace (localhost), and Volumes. No Service needed to communicate. Common pattern: main app + sidecar."},
          {q:"What is kubectl get pod <name> -o yaml used for?",options:["Deletes the Pod","Shows only errors","Displays the full Pod spec as YAML — useful for debugging and export","Restarts the Pod"],answer:2,explanation:"-o yaml outputs the complete Pod specification including status, settings, and metadata. Useful for inspecting config, exporting, or creating a similar resource."},
          {q:"What is the difference between a container and a Pod in Kubernetes?",options:["A Pod wraps one or more containers with a shared network, storage, and lifecycle","A Pod is a smaller container","Pod is for production, container is for dev","Pod is an image, container is an instance"],answer:0,explanation:"A container is an isolated runtime unit. A Pod is a Kubernetes abstraction grouping one or more containers that share an IP, volumes, and lifecycle."},
        ],
      },
      medium:{
        ...
        questions:[
          ...
          {q:"מה strategy: Recreate עושה ב-Deployment?",options:["מעדכן Pod אחד בכל פעם","מוחק את כל ה-Pods הישנים לפני שיוצר Pods חדשים – גורם לdowntime","יוצר namespace חדש","clone של Deployment"],answer:1,explanation:"Recreate מוחק את כל ה-Pods הישנים לפני שיוצר חדשים. מאפיין: downtime קצר. RollingUpdate מחליף בהדרגה ללא downtime."},
          {q:"מה maxUnavailable מגדיר ב-Rolling Update?",options:["מקסימום Nodes שמותרים לכשל","כמות Pods שמותרים להיות לא זמינים בו-זמנית בזמן עדכון","מגבלת זיכרון","מקסימום replicas"],answer:1,explanation:"maxUnavailable מגדיר כמה Pods יכולים להיות unavailable בו-זמנית בזמן rolling update. 0 אומר אפס downtime."},
          {q:"מה taints ו-tolerations פותרים?",options:["בעיות DNS","שליטה על איזה Pods מותרים לרוץ על Nodes מסוימים","בעיות TLS","בעיות storage"],answer:1,explanation:"Taint על Node מונע תזמון Pods שלא מגדירים Toleration תואם. משמש לייחוד Nodes ל-workloads מסוימים (GPU, spot instances)."},
          {q:"מה ההבדל בין requiredDuringScheduling ל-preferredDuringScheduling ב-affinity?",options:["אין הבדל","required חייב להתמלא אחרת Pod לא מתזמן; preferred מנסה אבל לא חייב","preferred לproduction בלבד","required לCPU, preferred לזיכרון"],answer:1,explanation:"requiredDuringSchedulingIgnoredDuringExecution: Pod לא יתזמן אם התנאי לא מתמלא (hard rule). preferredDuring...: מנסה למלא אבל לא חובה (soft rule)."},
          {q:"מה CronJob concurrencyPolicy: Forbid אומר?",options:["Job לא יוצר אף פעם","אם Job קודם עדיין רץ, Job חדש לא נוצר עד שהוא מסיים","Pods רצים במקביל תמיד","Job נמחק אוטומטית"],answer:1,explanation:"Forbid: אם Job קודם עדיין רץ, ה-CronJob מדלג על הריצה הבאה. Allow (ברירת מחדל): Jobs מקבילים. Replace: Job ישן נמחק ונוצר חדש."},
          {q:"מה Job backoffLimit מגדיר?",options:["זמן מקסימלי לריצה","מספר ניסיונות חוזרים לפני שJob מסומן כנכשל","מספר Pods מקסימלי","זמן בין ניסיונות"],answer:1,explanation:"backoffLimit מגדיר כמה פעמים Kubernetes ינסה מחדש לפני שJob מסומן Failed. ברירת מחדל: 6."},
          {q:"מה שלוש קלאסות QoS של Pod?",options:["Low, Medium, High","Guaranteed, Burstable, BestEffort","Bronze, Silver, Gold","Fast, Normal, Slow"],answer:1,explanation:"Guaranteed: requests=limits. Burstable: requests<limits. BestEffort: ללא requests/limits. בלחץ זיכרון, BestEffort מפונה ראשון."},
          {q:"מה kubectl rollout pause עושה?",options:["מוחק Deployment","עוצר rolling update בשלב נוכחי – שימושי לCanary deploys","מחזיר לגרסה קודמת","עוצר Pods מלקבל traffic"],answer:1,explanation:"kubectl rollout pause מאפשר להקפיא rollout באמצע. ניתן לשנות הגדרות נוספות ואז להמשיך עם kubectl rollout resume."},
          {q:"מה podAntiAffinity משמש לצורך?",options:["מושך Pods לNode ספציפי","מבטיח שPods לא יתוזמנו ביחד על אותו Node/Zone לזמינות גבוהה","מגביל traffic","מנהל DNS"],answer:1,explanation:"podAntiAffinity מונע מPods זהים לרוץ על אותו Node/Zone. שימושי לHA – פיזור replicas בין failure domains."},
          {q:"מה ephemeral container ב-Kubernetes?",options:["Pod זמני","קונטיינר שמוסיפים לPod רץ לdebug ללא restart","init container","גרסה מוקטנת של Pod"],answer:1,explanation:"Ephemeral containers מוספים לPod רץ דרך kubectl debug. שימושי כשה-image הראשי חסר כלי debug. אינם מאותחלים מחדש ולא מופיעים בspec."},
        ],
        questionsEn:[
          ...
          {q:"What does strategy: Recreate do in a Deployment?",options:["Updates one Pod at a time","Deletes all old Pods before creating new ones — causes downtime","Creates a new namespace","Clones the Deployment"],answer:1,explanation:"Recreate terminates all old Pods before starting new ones. Result: a short downtime window. RollingUpdate replaces gradually with no downtime."},
          {q:"What does maxUnavailable define in a Rolling Update?",options:["Max Nodes allowed to fail","Number of Pods allowed to be unavailable simultaneously during an update","Memory limit","Max replicas"],answer:1,explanation:"maxUnavailable controls how many Pods can be unavailable at once during a rolling update. Setting it to 0 means zero downtime."},
          {q:"What problem do taints and tolerations solve?",options:["DNS issues","Controlling which Pods are allowed to run on specific Nodes","TLS issues","Storage issues"],answer:1,explanation:"A taint on a Node prevents scheduling of Pods that don't declare a matching Toleration. Used to dedicate Nodes for specific workloads (GPUs, spot instances)."},
          {q:"What is the difference between requiredDuringScheduling and preferredDuringScheduling in affinity?",options:["No difference","required must be satisfied or the Pod won't schedule; preferred is a soft preference","preferred is production-only","required is for CPU, preferred for memory"],answer:1,explanation:"requiredDuringSchedulingIgnoredDuringExecution: Pod won't be scheduled if the rule isn't met (hard). preferredDuringScheduling: K8s tries but doesn't enforce (soft)."},
          {q:"What does CronJob concurrencyPolicy: Forbid mean?",options:["Job is never created","If the previous Job is still running, the new run is skipped","Pods always run in parallel","Job is deleted automatically"],answer:1,explanation:"Forbid: if the previous Job is still running, the next scheduled run is skipped. Allow (default): concurrent Jobs permitted. Replace: old Job is deleted and a new one created."},
          {q:"What does Job backoffLimit define?",options:["Maximum runtime","Number of retries before the Job is marked as Failed","Maximum Pod count","Time between retries"],answer:1,explanation:"backoffLimit sets how many times Kubernetes retries a failed Job before marking it Failed. The default is 6."},
          {q:"What are the three Pod QoS classes?",options:["Low, Medium, High","Guaranteed, Burstable, BestEffort","Bronze, Silver, Gold","Fast, Normal, Slow"],answer:1,explanation:"Guaranteed: requests=limits. Burstable: requests<limits. BestEffort: no requests/limits. Under memory pressure, BestEffort is evicted first."},
          {q:"What does kubectl rollout pause do?",options:["Deletes the Deployment","Freezes the rolling update at the current step — useful for canary deploys","Reverts to the previous version","Stops Pods from receiving traffic"],answer:1,explanation:"kubectl rollout pause freezes a rollout mid-way. You can make additional changes then resume with kubectl rollout resume."},
          {q:"What is podAntiAffinity used for?",options:["Attracts Pods to a specific Node","Ensures Pods are not co-scheduled on the same Node/Zone for high availability","Limits traffic","Manages DNS"],answer:1,explanation:"podAntiAffinity prevents identical Pods from landing on the same Node or Zone, spreading replicas across failure domains for HA."},
          {q:"What is an ephemeral container in Kubernetes?",options:["A temporary Pod","A container added to a running Pod for debugging without restarting it","An init container","A trimmed-down version of a Pod"],answer:1,explanation:"Ephemeral containers are injected into a running Pod via kubectl debug. Useful when the main image lacks debugging tools. They don't restart and don't appear in the Pod spec."},
        ],
      },
      hard:{
        ...
        questions:[
          ...
          {q:"Pod נשאר Pending. kubectl describe מראה:\nEvents:\n  Warning  FailedScheduling  0/3 nodes are available: 3 node(s) had untolerated taint {dedicated:gpu}.\nמה הפתרון?",options:["הוסף Node חדש","הקטן את ה-CPU request","הוסף toleration מתאים לPod spec","שנה Namespace"],answer:2,explanation:"ה-Node מסומן עם taint dedicated=gpu. Pod בלי toleration תואם לא יתוזמן שם. הוסף tolerations: [{key:'dedicated', value:'gpu', effect:'NoSchedule'}] ל-Pod spec."},
          {q:"StatefulSet עם 3 replicas. Pod-1 נשאר Pending. מה הסיבה הנפוצה?",options:["PVC מלא","Pod-0 לא Ready – StatefulSet מתזמן Pods בסדר לפי ordinal","Namespace quota","imagePull שגוי"],answer:1,explanation:"StatefulSet מתזמן Pods בסדר: pod-0 חייב להיות Ready לפני pod-1. בדוק kubectl get pod pod-0 ו-kubectl describe pod pod-0 לגלות מה חוסם."},
          {q:"kubectl get hpa מציג:\nNAME    REFERENCE         TARGETS         MINPODS  MAXPODS  REPLICAS\napp-hpa  Deployment/app   <unknown>/50%   2        10       2\nמה הסיבה ל-<unknown>?",options:["Deployment לא קיים","metrics-server לא מותקן – HPA לא מקבל מדדי CPU","CPU limit לא מוגדר","replicas=0"],answer:1,explanation:"<unknown> ב-TARGETS מציין שHPA לא מקבל metrics. הסיבה הנפוצה: metrics-server לא מותקן. בדוק: kubectl get apiservice v1beta1.metrics.k8s.io."},
          {q:"Rolling update נתקע. kubectl rollout status מציג:\nWaiting for rollout to finish: 3 out of 5 new replicas have been updated...\nה-YAML מגדיר maxUnavailable: 0. מה הסיבה?",options:["Namespace quota מלא","Pods החדשים לא עוברים readiness probe, ו-maxUnavailable:0 מונע הורדת ישנים","image שגוי","TLS שגוי"],answer:1,explanation:"עם maxUnavailable:0, K8s לא יכול להוריד Pod ישן עד שחדש Ready. אם readiness probe נכשלת, rollout נתקע. בדוק logs של Pod חדש."},
          {q:"DaemonSet לא מופיע על Node חדש. kubectl describe node מראה:\nTaints: node-role.kubernetes.io/control-plane:NoSchedule\nה-DaemonSet spec אינו כולל tolerations. מה הפתרון?",options:["מחק ה-Node","שנה Namespace","הוסף ל-DaemonSet spec: tolerations: [{key:'node-role.kubernetes.io/control-plane', effect:'NoSchedule'}]","הקטן DaemonSet requests"],answer:2,explanation:"Node עם taint NoSchedule דוחה Pods ללא Toleration תואם. DaemonSets ל-system components כמו logging צריכים לכלול את ה-toleration לcontrol-plane."},
          {q:"Pod חוזר ומקבל OOMKilled. kubectl describe pod מראה:\nLast State: Terminated Reason: OOMKilled Exit Code: 137\nContainers: limits: memory: 128Mi\nמה הצעד הראשון?",options:["מחק את ה-Pod","הגדל limits.memory ל-256Mi+ ובדוק אם האפליקציה דולפת זיכרון","הקטן replicas","הוסף liveness probe"],answer:1,explanation:"Exit 137 + OOMKilled = חריגה מlimits.memory. הגדל את הlimit. השתמש בkubectl top pod לראות שימוש אמיתי. בדוק גם memory leak באפליקציה."},
          {q:"Job לא הסתיים. kubectl describe job מציג:\nWarning  BackoffLimitExceeded  Job has reached the specified backoff limit\nמה קורה בנגינה?",options:["Job ממשיך לרוץ לנצח","Job מסומן כFailed – Kubernetes הפסיק לנסות מחדש","Pods נמחקים מיד","Job נמחק אוטומטית"],answer:1,explanation:"BackoffLimitExceeded אומר שJob נכשל backoffLimit פעמים ומסומן Failed. בדוק logs של ה-Pods שנכשלו כדי להבין הסיבה ולתקן."},
          {q:"Deployment לא מנהל Pods. kubectl get pods --show-labels מראה: app=backend-v2. ה-Deployment spec:\nspec:\n  selector:\n    matchLabels:\n      app: backend\nמה הבעיה?",options:["Namespace שגוי","selector לא תואם labels של Pods – 'backend' ≠ 'backend-v2'","image שגוי","Service חסר"],answer:1,explanation:"Deployment.spec.selector.matchLabels חייב להתאים ל-template.metadata.labels. אי-תאימות גורמת ל-Deployment לא לשלוט בPods. שנה selector או label."},
          {q:"kubectl rollout status deployment/app מציג:\nWaiting for rollout to finish: 1 old replicas are pending termination...\nזה נמשך 15 דקות. מה בודקים?",options:["image שגוי","Pod בTerminating בגלל finalizer, או terminationGracePeriodSeconds גבוה מדי","DNS שגוי","RBAC"],answer:1,explanation:"Pod בTerminating לאורך זמן = finalizer לא נוקה, או grace period ארוך. kubectl describe pod <terminating-pod> לראות finalizers. kubectl patch pod ... -p '{\"metadata\":{\"finalizers\":null}}' לביטול."},
          {q:"VPA ב-Recommendation mode. kubectl describe vpa app-vpa מציג:\nTarget: cpu: 450m, memory: 512Mi\nה-Pod רץ עם requests: cpu: 100m, memory: 128Mi. מה עושים?",options:["VPA מעדכן אוטומטית ב-Recommendation mode","ה-VPA רק ממליץ – יש לעדכן ידנית את ה-Deployment requests","מחק את ה-Pod","הגדל replicas"],answer:1,explanation:"VPA Recommendation mode רק מציג המלצות – לא מעדכן. Off/Initial/Auto מעדכנים. יש לעדכן ידנית את ה-Deployment לפי ההמלצה."},
        ],
        questionsEn:[
          ...
          {q:"A Pod stays Pending. kubectl describe shows:\nEvents:\n  Warning  FailedScheduling  0/3 nodes available: 3 node(s) had untolerated taint {dedicated:gpu}.\nWhat is the fix?",options:["Add a new Node","Reduce the CPU request","Add a matching toleration to the Pod spec","Change the Namespace"],answer:2,explanation:"The Node has a taint dedicated=gpu. A Pod without a matching Toleration won't be scheduled there. Add tolerations: [{key:'dedicated', value:'gpu', effect:'NoSchedule'}] to the Pod spec."},
          {q:"A StatefulSet with 3 replicas: Pod-1 stays Pending. What is the most likely cause?",options:["PVC is full","Pod-0 is not Ready — StatefulSet schedules Pods in ordinal order","Namespace quota exceeded","Wrong image"],answer:1,explanation:"StatefulSet schedules Pods sequentially: pod-0 must be Ready before pod-1 starts. Check kubectl get pod pod-0 and kubectl describe pod pod-0 to find the blocker."},
          {q:"kubectl get hpa shows:\nNAME    TARGETS         MINPODS  MAXPODS  REPLICAS\napp-hpa  <unknown>/50%   2        10       2\nWhat causes <unknown>?",options:["Deployment doesn't exist","metrics-server is not installed — HPA cannot receive CPU metrics","CPU limit not set","replicas=0"],answer:1,explanation:"<unknown> in TARGETS means HPA has no metrics. Most common cause: metrics-server is not installed. Check: kubectl get apiservice v1beta1.metrics.k8s.io."},
          {q:"A rolling update is stuck. kubectl rollout status shows:\nWaiting for rollout to finish: 3 out of 5 new replicas updated...\nThe YAML sets maxUnavailable: 0. What is the cause?",options:["Namespace quota full","New Pods are failing readiness probes, and maxUnavailable:0 prevents removing old ones","Wrong image","TLS misconfiguration"],answer:1,explanation:"With maxUnavailable:0, K8s can't remove an old Pod until a new one is Ready. If the readiness probe fails, the rollout stalls. Check logs of a new Pod."},
          {q:"A DaemonSet Pod is missing from a new Node. kubectl describe node shows:\nTaints: node-role.kubernetes.io/control-plane:NoSchedule\nThe DaemonSet has no tolerations. What is the fix?",options:["Delete the Node","Change the Namespace","Add to DaemonSet spec: tolerations: [{key:'node-role.kubernetes.io/control-plane', effect:'NoSchedule'}]","Reduce DaemonSet resource requests"],answer:2,explanation:"A NoSchedule taint blocks Pods without a matching Toleration. DaemonSets for system components like logging that need to run on control-plane nodes must include the matching toleration."},
          {q:"A Pod keeps getting OOMKilled. kubectl describe shows:\nLast State: Terminated  Reason: OOMKilled  Exit Code: 137\nContainers: limits: memory: 128Mi\nWhat is the first step?",options:["Delete the Pod","Increase limits.memory to 256Mi+ and check for memory leaks in the app","Reduce replicas","Add a liveness probe"],answer:1,explanation:"Exit 137 + OOMKilled = limits.memory exceeded. Increase the limit. Use kubectl top pod to see actual usage. Also profile the app for memory leaks."},
          {q:"A Job never completed. kubectl describe job shows:\nWarning  BackoffLimitExceeded  Job has reached the specified backoff limit\nWhat happens next?",options:["Job keeps running forever","Job is marked Failed — Kubernetes stops retrying","Pods are deleted immediately","Job is auto-deleted"],answer:1,explanation:"BackoffLimitExceeded means the Job failed backoffLimit times and is now marked Failed. Check the failed Pod logs to understand the root cause and fix the issue."},
          {q:"A Deployment does not manage its Pods. kubectl get pods --show-labels shows: app=backend-v2. The Deployment spec reads:\nspec:\n  selector:\n    matchLabels:\n      app: backend\nWhat is wrong?",options:["Wrong Namespace","selector doesn't match Pod labels — 'backend' ≠ 'backend-v2'","Wrong image","Missing Service"],answer:1,explanation:"Deployment.spec.selector.matchLabels must match template.metadata.labels. A mismatch means the Deployment doesn't control those Pods. Fix either the selector or the Pod label."},
          {q:"kubectl rollout status deployment/app shows:\nWaiting for rollout to finish: 1 old replica pending termination...\nThis has continued for 15 minutes. What do you check?",options:["Wrong image","Pod stuck in Terminating due to a finalizer or a high terminationGracePeriodSeconds","DNS misconfiguration","RBAC issue"],answer:1,explanation:"A Pod stuck Terminating usually means an uncleared finalizer or a long grace period. kubectl describe pod <terminating-pod> to inspect finalizers. kubectl patch pod ... -p '{\"metadata\":{\"finalizers\":null}}' to force-clear."},
          {q:"VPA is in Recommendation mode. kubectl describe vpa shows:\nTarget: cpu: 450m  memory: 512Mi\nThe Pod runs with requests: cpu: 100m  memory: 128Mi. What must you do?",options:["VPA updates requests automatically in Recommendation mode","VPA only recommends — update the Deployment requests manually","Delete the Pod","Increase replicas"],answer:1,explanation:"VPA Recommendation mode only shows suggestions — it does not apply them. Only Off/Initial/Auto modes update Pod specs. Update the Deployment requests manually according to the recommendation."},
        ],
      },
    }
  },
