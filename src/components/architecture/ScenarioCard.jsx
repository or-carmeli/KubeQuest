...
import { Database, TrendingUp, Search, RefreshCw, Zap, Globe, Lock, Clock } from "lucide-react";

const SCENARIO_ICONS = {
  "rds-latency":      <Database size={22} strokeWidth={1.5} />,
  "traffic-spike":    <TrendingUp size={22} strokeWidth={1.5} />,
  "lb-misconfigured": <Search size={22} strokeWidth={1.5} />,
  "pod-crash-loop":   <RefreshCw size={22} strokeWidth={1.5} />,
  "cache-stampede":   <Zap size={22} strokeWidth={1.5} />,
  "cdn-misconfigured": <Globe size={22} strokeWidth={1.5} />,
};
...
export default function ScenarioCard({ scenario, lang, onStart, bestScore, attempts, isDaily, locked = false }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
        <span style={{ color: locked ? "var(--text-dim)" : "var(--text-muted)", display: "flex", alignItems: "center" }}>
          {locked ? <Lock size={22} strokeWidth={1.5} /> : (SCENARIO_ICONS[scenario.id] || <Database size={22} strokeWidth={1.5} />)}
        </span>
        <span style={{
          background: diff.bg, color: diff.color, fontSize: 11, fontWeight: 700,
          padding: "3px 10px", borderRadius: 20, letterSpacing: 0.3,
        }}>
          {en ? diff.label : diff.labelHe}
        </span>
        {isDaily && !locked && (
          <span style={{
        ...
        <span style={{ fontSize: 11, color: "var(--text-muted)", marginInlineStart: "auto", display: "flex", alignItems: "center", gap: 4 }}>
      ...
      {/* Title */}
      <h3 style={{ margin: "0 0 6px", fontSize: 17, fontWeight: 800, color: locked ? "var(--text-muted)" : "var(--text-bright)", lineHeight: 1.4 }}>
        {title}
      </h3>
      ...
      {locked ? (
        <p style={{ margin: "0 0 14px", fontSize: 13, color: "var(--text-dim)", lineHeight: 1.6 }}>
          {en ? "Complete the previous scenario to unlock." : "השלם את התרחיש הקודם כדי לפתוח."}
        </p>
      ) : (
